...
public class ProductsDao extends BaseDao {
	...
	private String SqlProductPaginate( int start, int totalPage) {
		StringBuffer sql = SqlAllProduct();
		sql.append("LIMIT " + start + ", " + totalPage);
		return sql.toString();
	}
	public List<ProductsDto> GetDataProductPaginate(int start, int totalPage) {
		StringBuffer sqlGetDataByID = SqlAllProduct();
		List<ProductsDto> listProductsByID = _jdbcTemplate.query(sqlGetDataByID.toString(), new ProductsDtoMapper());
		List<ProductsDto> listProducts = new ArrayList<ProductsDto>();
		if(listProductsByID.size() > 0) {
			String sql = SqlProductPaginate(start, totalPage);
			listProducts = _jdbcTemplate.query(sql, new ProductsDtoMapper());
		}
		return listProducts;
